...
def create_announcement_by_teacher(
    ...
    # Handle class_code if it's provided in the request (for tests compatibility)
    if hasattr(announcement_in, 'class_code') and announcement_in.class_code:
        # If class_code is provided, find the corresponding class and set school_class_id
        db_class = crud.crud_school_class.get_school_class_orm_by_class_code(db, class_code=announcement_in.class_code)
        if not db_class:
            raise HTTPException(status_code=status.HTTP_404_NOT_FOUND,
                                detail=f"Class with code '{announcement_in.class_code}' not found.")

        # Set the school_class_id based on the found class
        announcement_in.school_class_id = db_class.id
