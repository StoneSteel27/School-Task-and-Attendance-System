...
from app.crud.auth.crud_user import get_user_by_email, update_user, get_user
from app.schemas.auth.token import Token
from app.db.session import get_db
from app.api import deps
from app.schemas.auth import webauthn as webauthn_schema
...
def webauthn_authentication_begin(
    *,
    ...
    """
    user = get_user(db, user_id=int(authentication_data.user_id))
    if not user:
...
def webauthn_authentication_finish(
    *,
    ...
    try:
        ...
        user = get_user(db, user_id=int(user_handle))
        if not user:
            raise HTTPException(status_code=404, detail="User not found")
