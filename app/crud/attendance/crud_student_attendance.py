...
from app.models.attendance.student_attendance import StudentAttendance, AttendanceSession, AttendanceStatus
from app.models.auth.user import User
from app.models.core.school_class import SchoolClass
from app.schemas.attendance.student_attendance import StudentAttendanceEntryInput  # For type hinting the entries
...
# Create a simple object to provide access to all student attendance CRUD functions
class CRUDStudentAttendance:
    """CRUD operations for StudentAttendance model"""
    
    def get_by_id(self, db: Session, attendance_id: int):
        return get_student_attendance_by_id(db, attendance_id)
    
    def get_attendance_for_student_on_date_session(self, db: Session, *, student_id: int, attendance_date, session):
        return get_attendance_for_student_on_date_session(db, student_id=student_id, attendance_date=attendance_date, session=session)
    
    def get_attendance_for_class_on_date_session(self, db: Session, *, school_class_id: int, attendance_date, session):
        return get_attendance_for_class_on_date_session(db, school_class_id=school_class_id, attendance_date=attendance_date, session=session)
    
    def create_class_attendance_batch(self, db: Session, *, school_class_id: int, marked_by_teacher_id: int, attendance_date, session, entries):
        return create_class_attendance_batch(db, school_class_id=school_class_id, marked_by_teacher_id=marked_by_teacher_id, attendance_date=attendance_date, session=session, entries=entries)
    
    def get_class_attendance_summary(self, db: Session, *, school_class_id: int, attendance_date, session):
        return get_class_attendance_summary(db, school_class_id=school_class_id, attendance_date=attendance_date, session=session)


# Create the instance that will be imported
student_attendance = CRUDStudentAttendance()
