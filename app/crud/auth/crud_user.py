...
# Create a simple object to provide access to all user CRUD functions
class CRUDUser:
    ...
    def get_by_roll_number(self, db: Session, roll_number: str):
        return get_user_by_roll_number(db, roll_number)

    def get_multi(self, db: Session, skip: int = 0, limit: int = 100):
        return get_users(db, skip, limit)

    def create(self, db: Session, *, user_in: UserCreate, password_hash: str):
        return create_user(db, user_in=user_in, password_hash=password_hash)

    def update(self, db: Session, *, db_user: User, user_in: UserUpdate):
        return update_user(db, db_user=db_user, user_in=user_in)

    def remove(self, db: Session, *, db_user: User):
        return remove_user(db, db_user_to_delete=db_user)

    def assign_student_to_class(self, db: Session, *, student: User, school_class):
        return assign_student_to_class(db, student=student, school_class=school_class)

    def unassign_student_from_class(self, db: Session, *, student: User):
        return unassign_student_from_class(db, student=student)

    def bulk_assign_students_to_class(self, db: Session, *, student_roll_numbers, target_school_class):
        return bulk_assign_students_to_class(db, student_roll_numbers=student_roll_numbers, target_school_class=target_school_class)

    def bulk_unassign_students_from_class(self, db: Session, *, student_roll_numbers, source_school_class):
        return bulk_unassign_students_from_class(db, student_roll_numbers=student_roll_numbers, source_school_class=source_school_class)

    def search_students_for_teacher(self, db: Session, *, teacher_class_ids, name=None, roll_number=None, class_code=None):
        return search_students_for_teacher(db, teacher_class_ids=teacher_class_ids, name=name, roll_number=roll_number, class_code=class_code)
