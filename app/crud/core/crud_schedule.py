...
from app.models.core.schedule import ClassScheduleSlot, Holiday  # DB Models
from app.schemas.core.schedule import (  # Pydantic Schemas
...
# Create a simple object to provide access to all schedule CRUD functions
class CRUDSchedule:
    """CRUD operations for Schedule and Holiday models"""

    def replace_schedule_for_class(self, db: Session, *, school_class_id: int, new_slots_data):
        return replace_schedule_for_class(db, school_class_id=school_class_id, new_slots_data=new_slots_data)

    def get_schedule_slots_for_class(self, db: Session, school_class_id: int):
        return get_schedule_slots_for_class(db, school_class_id)

    def get_schedule_slots_for_teacher(self, db: Session, teacher_id: int, day_of_week=None):
        return get_schedule_slots_for_teacher(db, teacher_id, day_of_week)

    def get_schedule_slot_by_id(self, db: Session, slot_id: int):
        return get_schedule_slot_by_id(db, slot_id)

    def create_holiday(self, db: Session, *, obj_in):
        return create_holiday(db, holiday_in=obj_in)

    def bulk_create_holidays(self, db: Session, *, holidays_in):
        return bulk_create_holidays(db, holidays_in=holidays_in)

    def get_holiday_by_id(self, db: Session, holiday_id: int):
        return get_holiday_by_id(db, holiday_id)

    def get_holidays_active_on_date(self, db: Session, *, target_date, grade_filter_value=None):
        return get_holidays_active_on_date(db, target_date=target_date, grade_filter_value=grade_filter_value)

    def get_holidays(self, db: Session, *, skip=0, limit=100, query_start_date=None, query_end_date=None, grade_filter_value=None):
        return get_holidays(db, skip=skip, limit=limit, query_start_date=query_start_date, query_end_date=query_end_date, grade_filter_value=grade_filter_value)

    def update_holiday(self, db: Session, *, db_obj, obj_in):
        return update_holiday(db, db_holiday=db_obj, holiday_in=obj_in)

    def delete_holiday(self, db: Session, *, holiday_id: int):
        return delete_holiday(db, holiday_id=holiday_id)
